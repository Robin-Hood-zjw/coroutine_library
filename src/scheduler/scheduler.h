...
namespace sylar {
    ...
    private:
        ...
        int _rootThread = -1;                                   // the ID of the root thread, used to determine whether the caller thread is included in the scheduler
        size_t _threadCount = 0;                                // the number of threads in the scheduler
        ...
        std::atomic<size_t> _idleThreadCount = {0};             // the number of idle threads in the scheduler, used to determine whether to tickle worker threads when new tasks are added
        std::atomic<size_t> _activeThreadCount = {0};           // the number of active threads in the scheduler, used for monitoring and debugging purposes

    protected:
        void SetThis();                                         // set the currently running scheduler

        virtual void tickle();                                  // tickle the worker threads to wake them up when new tasks are added to the scheduler
        ...
        bool hasIdleThreads() {return _idleThreadCount > 0;}    // check whether there are idle threads in the scheduler
    ...
    public:
        ...
        const std::string& getName() const {return _name;}                                              // get the name of the scheduler
        ...
        void scheduleLock(FiberOrCb fc, int thread = -1) {                                              
...

            {
                ...
                if (task.fiber || task.cb) _tasks.push_back(task);
            }
