...
extern "C"{
    ...
    int usleep(useconds_t usec) {
        if(!sylar::t_hook_enable) return usleep_f(usec);
    ...
    int nanosleep(const struct timespec* req, struct timespec* rem) {
        if (!sylar::t_hook_enable) return nanosleep_f(req, rem);
    ...
    int socket(int domain, int type, int protocol) {
        if(!sylar::t_hook_enable) return socket_f(domain, type, protocol);
    ...
    int connect_with_timeout(int fd, const struct sockaddr* addr, socklen_t addrlen, uint64_t timeout_ms) {
        if (!sylar::t_hook_enable) return connect_f(fd, addr, addrlen);
    ...
    int close(int fd) {
        if (!sylar::t_hook_enable) return close_f(fd);
...
// only use at task fiber
unsigned int sleep(unsigned int seconds) {
	if (!sylar::t_hook_enable) return sleep_f(seconds);
...
int accept(int sockfd, struct sockaddr *addr, socklen_t *addrlen) {
	int fd = do_io(sockfd, accept_f, "accept", sylar::IOManager::READ, SO_RCVTIMEO, addr, addrlen);	
	if (fd >= 0) sylar::FdMgr::GetInstance()->get(fd, true);
	return fd;
}
